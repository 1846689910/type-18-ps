...
export default class ValidationTemp extends React.Component {
  ...
  setInputStyle = (domInput, isCorrect) => {
    // 经典的表单背景框, 过审绿色，未过审红色
  ...
  validate = () => {
    ...
    this.setState({ valids }, () => {
      this.state.valids.forEach((obj, idx) =>
        Object.entries(obj).forEach(([k, v]) => this.setInputStyle(this[`${k}_${idx}`], v))
      );
    });
  };
  submit = () => {
        ...
        {this.state.dataset.map((item, idx) => (
          <div key={idx}>
            <label>
              ...
              <input
                type="text"
                ref={r => (this[`name_${idx}`] = r)}
                value={item.name}
                onChange={e => this.fieldChange("name", e.target, idx)}
              />
            </label>
            <label>
              min:{" "}
              <input
                type="number"
                ref={r => (this[`min_${idx}`] = r)}
                value={item.min}
                onChange={e => this.fieldChange("min", e.target, idx)}
              />
            </label>
            <label>
              max:{" "}
              <input
                type="number"
                ref={r => (this[`max_${idx}`] = r)}
                value={item.max}
                onChange={e => this.fieldChange("max", e.target, idx)}
              />
            </label>
          </div>
        ))}
